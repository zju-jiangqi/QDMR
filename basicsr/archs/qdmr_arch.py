...
from .qdmr_utils import ResBlock, CombineQuantBlock 
...
class ResidualDenseBlock(nn.Module):
    ...
    def __init__(self, num_feat, num_grow_ch=128):
...
class RRDB(nn.Module):
    ...
    def __init__(self, num_feat=256, num_grow_ch=128):
...
class RRDBLayers(nn.Module):
    def __init__(self, num_feat=256, num_grow_ch=128,
                **kwargs):
        super().__init__()
        self.rrdb_blks = nn.ModuleList()
        for i in range(4):
            layer = RRDB(num_feat, num_grow_ch, **kwargs)
            self.rrdb_blks.append(layer)
    
    def forward(self, x):
        # b, c, h, w = x.shape
        # x = x.reshape(b, c, h*w).transpose(1, 2)
        for m in self.rrdb_blks:
            x = m(x)
        # x = x.transpose(1, 2).reshape(b, c, h, w) 
        return x
...
class UDAVQGAN(nn.Module):
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        if self.use_semantic_loss:
            with torch.no_grad():
                vgg_feat = self.vgg_feat_extractor(img)[self.vgg_feat_layer]
        ...
        for i in range(self.max_depth):
            ...
            if cur_res in self.codebook_scale:  # needs to perform quantize
                ...
                # after_quant_feat = self.swinblock(after_quant_feat)
                ab_feat = after_quant_feat
        ...
        return out_img, codebook_loss, semantic_loss, indices_list, ab_feat
    ...
    def test(self, input):
        ...
        dec, _, _, _, _ = self.encode_and_decode(input)
    ...
    def forward(self, input, gt_indices=None):
        ...
        if gt_indices is not None:
            ...
            dec, codebook_loss, semantic_loss, indices, ab_feat = self.encode_and_decode(input)

        return dec, codebook_loss, semantic_loss, indices, ab_feat


@ARCH_REGISTRY.register()
class UDAVQGAN_S2T(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None,
...
@ARCH_REGISTRY.register()
class QDMR_UDA_Network(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None,
        ...
        self.vqfusion = SFT_layer(ch_fuse, ch_fuse)
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        #generate pretrained VQ Features
        before_quant_feat = cac_feats[0]
        ...
        #fuse ir feature and VQ feature
        # fused_feature = self.fuse_conv(torch.cat((cac_feats[0], after_quant_feat), dim=1))
        fused_feature = self.vqfusion(cac_feats[0], after_quant_feat)
        x = fused_feature
        ...
        out_img_residual = out_img
        codebook_loss = sum(codebook_loss_list)
        semantic_loss = sum(semantic_loss_list) if len(semantic_loss_list) else codebook_loss * 0

        return out_img_residual, codebook_loss, semantic_loss, return_feat
    ...
    def test(self, input, weight_alpha=None):
        ...
        out_img_residual, _, _, _ = self.encode_and_decode(input, None, None)
        output = out_img_residual
    ...
    def forward(self, input, gt_indices=None, weight_alpha=None):
        ...
        if gt_indices is not None:
            # in LQ training stage, need to pass GT indices for supervise.
            out_img_residual, codebook_loss, semantic_loss, return_feat = self.encode_and_decode(input, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            out_img_residual, codebook_loss, semantic_loss, return_feat = self.encode_and_decode(input)
...
@ARCH_REGISTRY.register()
class QDMR_Base_Network(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None,
    ...
    def forward(self, input, gt_indices=None, weight_alpha=None):
        ...
        if gt_indices is not None:
            # in LQ training stage, need to pass GT indices for supervise.
            dec, dec_residual, codebook_loss, semantic_loss, quant_before_feature, quant_after_feature, indices = self.encode_and_decode(input, gt_indices)
        else:
            # in HQ stage, or LQ test stage, no GT indices needed.
            dec, dec_residual, codebook_loss, semantic_loss, quant_before_feature, quant_after_feature, indices = self.encode_and_decode(input)
