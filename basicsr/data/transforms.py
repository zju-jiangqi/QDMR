...
# def abs_random_crop(img_gts, img_lqs, psf, gt_patch_size, scale, gt_path):
#     """Paired random crop.
...
#     psf = [
#         p[:,top:top + lq_patch_size, left:left + lq_patch_size, ...]
#         for p in psf
#     ]
...
#     if len(psf) == 1:
#         psf = psf[0]
#     return img_gts, img_lqs, psf

# def absn_random_crop(img_gts, img_lqs, img_mqs, psf, gt_patch_size, scale, gt_path):
#     """Paired random crop.
...
#     if not isinstance(img_lqs, list):
#         img_lqs = [img_lqs]
#     if not isinstance(img_mqs, list):
#         img_mqs = [img_mqs]
#     if not isinstance(psf, list):
#         psf = [psf]
...
#     top_mq, left_mq = int(top * scale), int(left * scale)
#     img_mqs = [
#         v[top_mq:top_mq + gt_patch_size, left_mq:left_mq + gt_patch_size, ...]
#         for v in img_mqs
#     ]
#     if len(img_gts) == 1:
#         img_gts = img_gts[0]
...
#     if len(psf) == 1:
#         psf = psf[0]
#     return img_gts, img_lqs, img_mqs, psf

# def augment(imgs, psfs=None, hflip=True, rotation=True, flows=None, return_status=False):
#     """Augment: horizontal flips OR rotate (0, 90, 180, 270 degrees).
...
#             psfs = [psfs]
#         psfs = [_augmentpsf(psf) for psf in psfs]
#         if len(psfs) == 1:
#             psfs = psfs[0]
...
#             flows = flows[0]
#         if psfs is not None:
#             return imgs, psfs, flows
#         else:
#             return imgs, flows
#     else:
#         if return_status:
#             return imgs, psfs, (hflip, vflip, rot90)
#         else:
#             if psfs is not None:
#                 return imgs, psfs
#             else:
